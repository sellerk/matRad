...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    properties         
        ...
        parallelRuns = true; %Starts runs in parallel
        ...
        numOfRuns = 1; %Default number of runs / batches % temp MaW for fast check
        modeHistories = 'num'; %'frac';
        fracHistories = 1e-4; %Fraction of histories to compute
        numHistories = 1e5; %Number of histories to compute        
        ...
        minRelWeight = 0; %.00001; %Threshold for discarding beamlets. 0 means all weights are being considered, can otherwise be assigned to min(w)
              
        useOrigBaseData = true; % base data of the original matRad plan will be used?
        beamProfile = 'simple'; %'biGaussian'; %'biGaussian' (emittance); 'simple'
        ...
        CutForAllParticles_inPatient = 0.001; %in mm %temp MaW 
        ...
        infilenames = struct(   'geometry','TOPAS_matRad_geometry.txt.in',...                               
                                ...
                                'graphics', 'TOPAS_graphics.txt.in'... %temp MaW
                            );
        %Graphics 
        enableGraphics = false;  %use graphical implementation to show the current simulation
    ...
    methods
        ...
        function writeAllFiles(obj,ct,pln,stf,topasBaseData,w)
            ...
            obj.MCparam.Loopcounter = ''; % temp MaW
    ...
    methods (Access = private)
        ...
        function writeFieldHeader(obj,fID,fieldIx)
            ...
            fprintf(fID,'d:Ph/Default/ForRegion/Inpatient/CutForAllParticles = %f mm\n',obj.CutForAllParticles_inPatient); %temp MaW 
        ...
        function writeGraphics(obj, fID)
             matRad_cfg = MatRad_Config.instance(); %Instance of matRad configuration class
            
            if obj.enableGraphics
                fname = fullfile(obj.thisFolder,obj.infilenames.graphics);
                matRad_cfg.dispInfo('Reading Graphics output preferences from %s\n',fname);
                graphics = fileread(fname);
                fprintf(fID,'%s\n',graphics);
            end
        end
        ...
        function writeStfFields(obj,ct,stf,baseData,w)
            ...
            for beamIx = 1:length(stf)
                ...
                for rayIx = 1:stf(beamIx).numOfRays
                    for bixelIx = 1:stf(beamIx).numOfBixelsPerRay(rayIx)
                        ...
                        if (nCurrentParticles>minParticlesBixel)
                            ...
                            disp(['skipped ', num2str(nCurrentParticles)])
                        end
                ...
                if any(historyCount < cutNumOfBixel) || cutNumOfBixel == 0
        ...
        function writePatient(obj,ct,pln)
            ...
            obj.MCparam.Loopcounter = obj.Loopcounter; % temp MaW
            ...
            switch cubeExport
                ...
                case 'directDens' 
                    ...
                    for ix=1:length(unique_hu)
                        unique_materials{ix} = strrep(['Material_HU_',num2str(unique_hu(ix))],'-','m');
                        fprintf(fID,'s:Ma/%s/BaseMaterial = "%s"\n',unique_materials{ix},medium);
                        fprintf(fID,'d:Ma/%s/Density = %f g/cm3\n',unique_materials{ix},unique_hu(ix));
                    end
