...
% catch if current modulation depth is 0 => no modulating material in
...
%     normvert(:,3) = ( 1 / sqrt(2*pi()*sigma.^2) ) * exp(- ((mu - normvert(:,1)).^2)/(2*sigma.^2) );
    % Convolution
%     try 
        conv_res = convnfft(baseData.Z_adapted, normvert(:,2), 'same');
%     catch
%         conv_res = conv(baseData.Z_adapted, normvert(:,2), 'same');
%     end
